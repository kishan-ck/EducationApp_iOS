...
class APIClient {
    ...
    /// Common function that is used to call all the apis
    func callAlamofireURLSessionService(serviceName: APIEndPoint, parameters: Parameters, method: HTTPMethod, isShowLoader: Bool = true, withSuccess: @escaping ((_ responseObj: json?) -> Void), failure: @escaping ((_ error: String?) -> Void)){
        
        let pageUrlStr: String = Config.verifyReceiptURL + serviceName.value
        ...
        guard let urls = URL(string: pageUrlStr) else { return }
        var request = URLRequest(url: urls, timeoutInterval: Double.infinity)
        ...
        let task = URLSession.shared.dataTask(with: request) { data, response, error in
            guard let data = data else {
                ...
                print("Error verifying receipt: \(error?.localizedDescription ?? "Unknown error")")
                return
            }
            if(isShowLoader){
                DispatchQueue.main.async {
            ...
            if let jsonData = try? JSON(data: data) {
                ...
                if let httpResponse = response as? HTTPURLResponse {
                    jsonDictionary?["statusCode"] = json(httpResponse.statusCode)
                }
                
                if let dic = jsonDictionary {
                    //print("JSON Dictionary :- ", dic)
                }
